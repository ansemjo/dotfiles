...
# Docker's GetRandomName()
# Source: https://github.com/moby/moby/blob/master/pkg/namesgenerator/names-generator.go
#
# Rewritten for use in bash shell.
# Use these regular expressions to convert lists for bash:
#
#   :'<,'>s/^\t\t\/\//\t\t#/     (convert // comments to #)
...
randomname() {
  ...
  while true; do
    ...
    # get random items
    l=${ADJECT[ $RANDOM$RANDOM % ${#ADJECT[@]} ]}
    r=${PERSON[ $RANDOM$RANDOM % ${#PERSON[@]} ]}
    c=${1:--}; c=${c::1}

    # steve wozniak is not boring
    if [[ $l-$r == boring-wozniak ]]; then continue; fi
